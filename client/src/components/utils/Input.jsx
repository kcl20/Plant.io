...
export const Text = ({ id, name, type, value, className = "", placeholder, onChange }) => {
  return (
    <input id={id} type={type} name={name} value={value} className={`block w-full h-20 mt-2 px-3 py-2 text-gray-600 rounded-[4px] border-2 border-gray-100 focus:border-primary transition outline-none hover:border-gray-300 ${className}`} placeholder={placeholder} onChange={onChange} />
  )
}
